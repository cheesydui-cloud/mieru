...
### 查询页下载区位置
- 客户文件 **Downloads / 文件下载** 区块移到页面最下方（订阅区之后）。

## v0.5.26

### 客户文件上限 500MB
- 单文件上传上限由 50MB 调整为 **500MB**（仍走分片上传）。
...
### 总览参考式流量看板
- KPI 卡片参考常见转发面板：轻卡片、右上角图标、底部进度条。
- 24 小时流量图：完整 00:00–23:00 刻度、网格、悬浮时间/流量提示框。
- 保留现有指标含义（本月总流量 / 今日 / 节点 / 待处理）与墨绿运维色。

## v0.5.22

### 查询页品牌
- 去掉品牌下的「账号信息 / Account」副标题。
- 面板名称完整显示（不再省略号截断）。
- 墨绿下划线加粗，并与名称同宽。
...
### 查询页品牌与视觉
- 说明：面板中文名是自定义品牌，不会自动翻译；可另设 **英文面板名称**（`panel_name_en`），查询页 English 时使用。
- 查询页品牌文案下增加墨绿色短横线。
- 管理端整体去「产品/AI」感：更小圆角、去软阴影与胶囊按钮，保留现有结构与功能。

## v0.5.20

### 用户查询页中英文
- 全局设置 `user_info_locale`（zh / en）：用户打开查询页时整页切换中英文。
- **用户**页工具栏一键 **中文 / EN**；设置页「面板」区亦可切换并随品牌保存。
- 查询页 UI 文案（状态、流量、扫码、订阅、公告控件等）双语；公告内容仍按管理员原文。

## v0.5.19

### UI 精简
- 去掉顶栏多余的「用户名 · 页面名」（桌面端隐藏顶栏；移动端仅保留菜单按钮）。
- 用户行操作只保留 **扫码 / 复制查询页 / 更多**；编辑、续期、启停、倍率、加流量、重置等收入更多；**删除** 置底。

## v0.5.18

### 总览今日流量曲线
- KPI 下方新增 **今日按小时流量图**（X：本地 0–23 点，Y：该小时总流量）。
- 数据来自 `traffic_hourly`，按本地日边界映射 UTC 小时桶。
- 悬停显示该小时总量与上下行；无额外图表库（SVG）。

## v0.5.17

### 总览 KPI
- 「启用隧道」改为 **总流量**（当月累计，本地每月 1 日 0 点起算）。
- KPI 顺序：**总流量 · 今日流量 · 节点在线 · 待处理**。

### 品牌
- Logo 方块与默认 favicon 底色由黑改为墨绿青 accent。

### 操作审计
- 默认只看重要操作（隐藏登录 / 登录失败 / 启动自动重建）。
- 中文动作名、操作者并入操作列；详情截断；最多展示 50 条。
- 搜索可找回被隐藏的记录。
...
### 总览页精简
- 去掉 12 张指标卡堆叠与重复的隧道表。
- 顶部仅 **4 个主 KPI**（在线 / 今日流量 / 隧道 / 待处理）。
- 异常项收敛为可点击 **alert chips**；问题列表独立成块，无问题时不占位。
- 隧道拓扑保留；节点明细改为精简状态表，完整运维仍在「节点」页。

## v0.5.15

### 管理端视觉精修（浅色 · 墨绿青）
- 设计 token：冷灰底 + 墨绿青 accent，更小圆角与更紧信息密度。
- Logo 默认字母固定为 **M**（黄金比例字号/圆角），不再取面板中文名首字（如「专」）。
- 侧栏去掉副标题 hint 与营销文案卡；导航更干净。
- 节点页：行操作收敛为「编辑 + ⋯」菜单；批量操作收进下拉；去掉页顶长说明。
- 登录页品牌区与状态色/焦点环统一到新 token。
- **用户查询页布局未改**（仅默认 favicon 字母随全局 brand 变为 M）。

## v0.5.14

### 精简节点安装与状态展示
- 安装命令**仅保留 Agent 一键安装**，去掉 ufw / firewall-cmd 放行片段与「安装+防火墙」文案。
- 节点列表：Agent 已上报与设置一致的 `PANEL_URL` 时，清除「纠正面板地址」挂起角标与内存任务，避免假阳性。
- 离线节点仍可「复制修复命令」；在线漂移由心跳自动纠正，列表不再额外塞修复按钮。
